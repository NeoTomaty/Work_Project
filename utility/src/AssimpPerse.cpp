...
namespace AssimpPerse
{
	...
	std::vector<std::unique_ptr<Texture>> g_diffuseTextures;	// ディフューズテクスチャ群
	...
	TreeNode<std::string>	g_AssimpNodeNameTree;				// assimpノード名ツリー

	// ノード名ツリーを生成する
	void CreateNodeTree(aiNode* node, TreeNode<std::string>* ptree) {

		ptree->m_nodedata = std::string(node->mName.C_Str());

		for (unsigned int n = 0; n < node->mNumChildren; n++)
		{
			std::unique_ptr<TreeNode<std::string>> pchild = std::make_unique<TreeNode<std::string>>();
	...
	TreeNode<std::string> GetBoneNameTree() 
	{
...
//			int vertexbase = g_subsets[subsetid].VertexBase;
	...
	// ディフューズCTextureコンテナを返す
	std::vector<std::unique_ptr<Texture>> GetDiffuseTextures()
	{
		return std::move(g_diffuseTextures);
	}

	// マテリアル情報をassimpを使用して取得する
	void GetMaterialData(const aiScene* pScene)
	{
	...
	void GetModelData(std::string filename)
	{
		// シーン情報構築
		...
		g_indices.clear();
		g_materials.clear();
		g_diffuseTextures.clear();
		g_subsets.clear();
		g_BoneDictionary.clear();
		g_BonesPerMeshes.clear();

		// マテリアル情報取得
		GetMaterialData(pScene);
		...
		for (unsigned int m = 0; m < pScene->mNumMeshes; m++)
		{
			...
			for (unsigned int vidx = 0; vidx < mesh->mNumVertices; vidx++)
			{
				...
				if (mesh->HasTextureCoords(0)) {
					...
					v.texcoord.y = 1.0f - v.texcoord.y; // UV座標のv成分を反転
				}
				else
				{
				...
				if (mesh->HasTangentsAndBitangents()) {
					v.tangent = mesh->mTangents[vidx];


				}
				else {
					v.tangent = aiVector3D(0.0f, 0.0f, 0.0f);
				}
