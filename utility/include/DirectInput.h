...
private:
	...
	HWND					m_hwnd{};
	DirectInput() :m_dinput(nullptr), m_dikeyboard(nullptr), m_dimouse(nullptr) {
	}
public:
	...
	POINT GetMouseDelta() {
		...
		// 前回のマウス位置と現在のマウス位置の差分を計算
		POINT delta = { currentPos.x - m_MousePoint.x, currentPos.y - m_MousePoint.y };

		// 現在のマウス座標を更新
		m_MousePoint = currentPos;

		return delta; // 移動量（デルタ）を返す
	}
};
